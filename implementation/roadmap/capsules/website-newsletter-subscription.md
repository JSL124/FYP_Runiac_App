...
- `tests/firebase-rules`: newsletter deny-all suite (`newsletter.firestore.rules.test.mjs`) is enumerated in `tests/firebase-rules/package.json` and therefore runs inside the hosted `backend-emulator-tests` job.
  - **Not verified at ship time (2026-07-30).** Commit `d3677cdb` has no hosted CI run of its own — `gh run list --commit d3677cdb` returns empty — so no ship-time pass can be claimed for it.
  - **Verified afterwards on `main`.** At `6eb6efef` the `backend-emulator-tests` job passed the full Firestore/Storage rules family, this suite included: hosted run [`30568823947`](https://github.com/JSL124/Runiac_App/actions/runs/30568823947), 2026-07-30T18:04:38Z.
  - Evidence rule applied here: a suite being registered in `package.json` is **not** evidence that it ran. Only a hosted CI run is. Verified 2026-07-31.
