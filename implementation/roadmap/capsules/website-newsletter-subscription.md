...
Routed on 2026-07-30 Asia/Singapore. Not yet implemented: this capsule and its governance-CI allowlist wiring exist so a scoped `functions/src/newsletter/**` and `firestore.rules`/`firestore.indexes.json` diff can land without tripping the diff-hygiene gate. No Cloud Function, rule, or index has been written or deployed under this capsule as of this record.
