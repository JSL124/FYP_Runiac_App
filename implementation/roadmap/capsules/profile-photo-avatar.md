...
- Deployed from the then-uncommitted working tree on branch `feat/profile-photo-avatars`. The commit was authorized immediately after and is `3cd62d4a` (`feat(profile): let runners set a profile photo across every surface`). The deploy preceded its own commit in wall-clock order; the hash is a post-hoc attestation that the deployed tree and that commit match, not a provenance record produced by the build.
