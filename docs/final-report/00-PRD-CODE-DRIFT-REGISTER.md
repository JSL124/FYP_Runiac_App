...
### C3. Wearable and health integration is not implemented — GAP / DECLARE
...
`pubspec.yaml` contains no health, HealthKit or Health Connect package. Sensor input is `geolocator` for GPS and `sensors_plus` for motion. Heart-rate zone analysis is therefore not delivered either: the analysis classes exist but have no data source to read.

**Evidence updated 13 Aug 2026 — the verdict is unchanged, the supporting detail is not.** This entry previously cited a "Watch and health apps" configuration screen and the `healthWorkoutImport` feature key as things that existed without a device SDK behind them. Both are gone. Between the two, a native iOS import chain was in fact built (`RuniacHealthKitImportChannel.swift` and an Apple Health repository, 20 Jun 2026) and then removed whole on 6 Aug 2026 in `65b41c49`, precisely because it was never finished and the app was advertising a capability it could not deliver. The feature key outlived it in `DEFAULT_FEATURE_ACCESS_CONFIG` and the admin console until 13 Aug 2026. Verified against the working tree on that date: no health file in `ios/Runner/`, an empty `Runner.entitlements`, and no `healthWorkoutImport` in the app source or the entitlement catalog. See `00-SOURCE-CODE-DRIFT-REGISTER.md` C3, which was wrong in both directions before settling here.

*Report treatment:* this is the most significant unfulfilled claim in the PRD and must be declared clearly in Chapters 2 and 10 rather than left for an assessor to discover. The comparison table in Chapter 8 must be corrected before reuse. Chapter 1 Risk R3 has been corrected from "mitigated on the delivered path" to "did not arise".
